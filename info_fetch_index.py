...
Modern PyTerrier optimizations:
  - tokeniser="whitespace": strictly splits on spaces without re-filtering
  - meta={"docno": 10}: stores minimal docno metadata, avoiding payload caching
  - Streams generator directly into indexer without loading full corpus into memory

Outputs:
  - info_fetch_terrier_index/ : Directory containing the inverted index files
  - info_fetch_index_stats.txt: Collection statistics and indexing time/size
...
def get_dir_size_kb(path: str) -> float:
    """Calculate directory size on disk in kilobytes."""
    total = 0
...
def main():
    ...
    print(f"\nBuilding index at '{INDEX_DIR}' directly from stream '{PROCESSED_FILE}' ...")
    print("  - Tokenizer: whitespace (preserves PA1 tokens & stems)")
    print("  - Metadata: docno only (lean, memory-efficient index)")
    print("  - Stemmer / Stopper: None (PA1 pipeline already applied)")
    ...
    t_start = time.time()
    # Stream directly from generator without in-memory list caching
    index_ref = indexer.index(parse_processed_corpus(PROCESSED_FILE))
